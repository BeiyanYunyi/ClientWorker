...
const clientworkerhandle = async (request: Request) => {
  ...
  const config = await db.read<ConfigType>("config", {
    type: "json",
  });
  ...
  for (let catch_rule of config.catch_rules) {
    ...
    for (let transform_rule of catch_rule.transform_rules) {
      ...
      switch (transform_rule.searchin || "url") {
        ...
        case "body":
          if (!tFetched) {
            cons.w(`${tReq.url} is not fetched yet,the body rule are ignored`);
            break;
          }
          if (
        ...
        switch (transform_rule.action) {
          ...
          case "fetch":
            ...
            if (typeof transform_rule.fetch === "undefined") {
              cons.e(`Fetch Config is not defined for ${tReq.url}`);
              break;
            }
                  ...
                  if (!EngineFetch) {
                    ...
                    switch (transform_rule.fetch?.engine || "parallel") {
                      case "classic":
                        ...
                        break;
                      case "parallel":
                        cRes = await FetchEngine.parallel(
                          EngineFetchList,
                          fetchConfig!
                        );
                        break;
                      case "KFCThursdayVW50":
                        ...
                        cRes = await FetchEngine.KFCThursdayVW50(
                          EngineFetchList,
                          fetchConfig!
                        );
                        break;
